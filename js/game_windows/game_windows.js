...
create_predefined_windows() {

	this.tid_wnd_shop = setTimeout(() => {
		if (!this.wnd_shop) {
			this.wnd_shop = new BuyMoney(this.scene);
			this.wnd_shop.init({});
			this.wnd_shop.visible = false;
		}
		this.tid_wnd_shop = null;
	}, 100);
...
show_window(params, timeout = 0) {
	...
	let use_push = true;
	if (id == 'level_start') {
		if (this.pending_windows.length || this.wind_id == id) allow = false;
	}
	if (params && params['sale'] && this.pending_windows.length > 0) allow = false;
	if (id == 'challenge_duel' && (this.pending_windows.length > 0 || this.game_window)) allow = false;
	if (params && params['sale'] && (this.pending_windows.length > 0)) allow = false;
	if (params && allow) {
...
display_window(params) {
	...
	switch (window_id) {
		...
		break;
		case 'level_start':
			this.game_window = new LevelStart(this.scene);
		break;
		case 'level_finished':
			this.game_window = new LevelFinished(this.scene);
		break;
		case 'remove_ads':
...
display_smooth(center_pt, appear_type) {			
	...
	if (appear_type == 'center') {				
		...
		game_data['scene'].tweens.add({targets: this.game_window, y: center_pt.y, ease: 'Sine.easeInOut', duration: 500, onComplete: function(){
		}});  
	}
},
...
close_window(params = null) {
	...
	if (this.allow_window_close) {
		...
		if (params && 'immediate' in params) {
			...
			game_data['scene'].tweens.add({targets: this.game_window, x: dx + 1000, duration: 500, ease: 'Sine.easeInOut', onComplete: () => {
				if (this.wind_id == _id) this.destroy_window();	 
			}}); 
			game_data['audio_manager'].sound_event({'play': true, 'sound_name': 'window_open'});	
		}
...
destroy_window(block_resume = false) {
	if (this.wind_id == 'buy_money') {
		this.game_window.visible = false;
		this.add(this.game_window);
	}
	else if (this.game_window) this.game_window.destroy();
